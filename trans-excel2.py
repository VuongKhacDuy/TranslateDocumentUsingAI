...
9. Translate all segments separated by "|||" and keep them separated with the same delimiter"""
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gemini-2.0-flash", # Or "gemini-pro" or other suitable model
            messages=[
...
def validate_api_key():
    """Validate the API key before starting translation"""
    api_key = os.getenv("GEMINI_API_KEY")
    if not api_key:
        print("❌ Error: GEMINI_API_KEY not found in .env file")
        print("Please add your Gemini API key to the .env file with format:")
        print("GEMINI_API_KEY=your_api_key_here")
        return False
    if api_key == "your_api_key_here":
        print("❌ Error: Please replace the default API key with your actual Gemini API key in .env file")
        return False
    return True
